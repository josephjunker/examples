...
// begin-snippet: cps-traversals
function flattenTreePreCPS<T, K>(
    tree: BinaryTree<T>,
    ...
    function inner(
        ...
        k: (res: LinkedList<T>) => K): K {
            ...
            if (!tree) return k(acc0);

            const acc1 = concat(tree.contents, acc0);
            return inner(tree.left, acc1, (acc2) =>
                inner(tree.right, acc2, k));
        }

    return inner(tree, emptyList(), k);
}

export function flattenTreePostCPS<T, K>(
    tree: BinaryTree<T>,
    k: ((result: LinkedList<T>) => K)): K {
    ...
    function inner(
        ...
        return inner(tree.right, acc0, (acc1) =>
            inner(tree.left, acc1, (acc2) =>
                k(concat(tree.contents, acc2))));
    }
...
test_traversals("pre-order equalities", () => {
    fc.assert(
        fc.property(binaryTree, tree => {
            ...
            const preCPSTree = flattenTreePreCPS(tree, x => x);

            assert.equal(preIterTree, preRecTree);
            assert.equal(preRecTree, preCPSTree);
        }), { numRuns: 500 });
});
